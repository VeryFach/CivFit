// Conditional import for expo-sqlite - only on native platforms
let SQLite: any;
...
class SQLiteDatabase {
    ...
    async execute(query: string, params: any[] = []) {
        ...
        if (!this.db) {
            console.warn('[SQLite Mock] execute() called but no database');
    ...
    async query<T>(query: string, params: any[] = []): Promise<T[]> {
        if (!this.db) await this.init();
        if (!this.db) {
            console.warn('[SQLite Mock] query() called but no database');
            return [];
        }
    ...
    async transaction(callback: () => Promise<void>) {
        if (!this.db) await this.init();
        if (!this.db) {
            console.warn('[SQLite Mock] transaction() called but no database');
            await callback();
            return;
        }
